      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-12">
        <AnimatePresence mode="popLayout">
          {filteredItems.map((item) => (
            ...
            >
              <div className="aspect-[4/5] relative overflow-hidden bg-bg-off-white border border-border-color p-2 mb-8 bg-white">
                {item.photo_url ? (
                  <img 
                    src={item.photo_url} 
                    alt={item.name} 
              ...
              <div className="flex flex-col flex-grow space-y-4 text-center">
                <div className="space-y-2">
                  <h3 className="text-xl font-display italic text-main-text leading-tight group-hover:text-accent-red transition-colors duration-500">{item.name}</h3>
                  <p className="text-secondary-text text-xs font-serif italic line-clamp-2 h-8">
                    {item.description}
                  </p>
                </div>
                
                <div className="pt-2">
                  <span className="text-accent-red font-sans font-bold text-sm tracking-widest">
                ...
                <button
                  ...
                  className={`w-full py-4 text-[10px] font-sans font-bold uppercase tracking-[0.3em] transition-all duration-500 border ${
                    addedId === item.id 
